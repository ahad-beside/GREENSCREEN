...
<table cellpadding="3" cellspacing="3" style="width: 100%;">
	...
	$nsql = "SELECT COUNT(*) AS nrow FROM leads WHERE `call` != '3' AND (`marks_data` = '0000-00-00' OR `marks_data` <= '$currentDate') AND rep_id = '".$rep_id."' ORDER BY `lead_id`";
    $nresult = mysql_query($nsql, $conn);
    $fetchn = mysql_fetch_array($nresult);
    $ncount = $fetchn['nrow'];
	
	if($_GET['lead_id']!='') {
        $sql = "SELECT leads.*, reps.name,reps.rep_id AS code FROM leads INNER JOIN reps ON reps.id = leads.rep_id WHERE leads.lead_id = '".intval($_GET['lead_id'])."' AND leads.call != '3' AND (leads.`marks_data` = '0000-00-00' OR leads.`marks_data` <= '$currentDate') AND leads.rep_id = '".$rep_id."'  ORDER BY leads.`lead_id` ASC limit 1";
    }else {
        $sql = "SELECT leads.*, reps.name,reps.rep_id AS code FROM leads INNER JOIN reps ON reps.id = leads.rep_id WHERE leads.call != '3' AND (leads.`marks_data` = '0000-00-00' OR leads.`marks_data` <= '$currentDate') AND leads.rep_id = '".$rep_id."' ORDER BY leads.`lead_id` ASC limit 1";
    }
    $result = mysql_query($sql, $conn);
    $fetch = mysql_fetch_array($result);

    $presql = "SELECT lead_id from leads where lead_id<". $fetch['lead_id'] ." and leads.call != '3' AND (leads.`marks_data` = '0000-00-00' OR leads.`marks_data` <= '$currentDate') AND leads.rep_id = '".$rep_id."' order by lead_id desc limit 1";
    $resultp = mysql_query($presql, $conn);
    $fetchp = mysql_fetch_array($resultp);

    $nextsql = "SELECT lead_id from leads where lead_id>". $fetch['lead_id'] ." and leads.call != '3' AND (leads.`marks_data` = '0000-00-00' OR leads.`marks_data` <= '$currentDate') AND leads.rep_id = '".$rep_id."' order by lead_id ASC limit 1";
...
<tr>	
	...
	<div style="text-align: center;">
		...
		<a style="float: right; margin-top:-50px;" id="modalForm1" href="#modelForm1<?php echo $lead_id ;?>">
...
<div style="text-align: center;">
	<form method="GET" action="">
        <div id="div_pagination">
			...
			<button type="button" class="self_call" id="<?php echo $lead_id; ?>" name="but_call">Delete</button><?php ?>
 			<button type="button" class="self_sell" id="<?php echo $lead_id; ?>" name="but_sell">Sale/No Sale</button>                
          	</div>
        	<div style="float: right; margin-bottom: 10px; margin-top: -8px;">
			
			<ul class="pagination">
        		<?php if($fetchp['lead_id']!=''):?>
        			<a href="list_self.php?lead_id=<?= $fetchp['lead_id']?>"><button type="button">Previous</button></a>
        		<?php endif;?>
        		<?php if($fetchn['lead_id']!=''):?>
        		<a href="list_self.php?lead_id=<?= $fetchn['lead_id']?>"><button type="button">Next</button></a>
        		<?php endif;?>
			</ul>        	
...
<div id="clientForm1" style="display:none; width:600px;">
<form name="newClient" id="#modelForm1<?php echo $lead_id ;?>" method="post" action="">
	...
	<tr>
		<td colspan="2" style="text-align:center;"><input type="button" id="newClientModal1" name="submitButton" value="Update Client" />&nbsp;&nbsp;
