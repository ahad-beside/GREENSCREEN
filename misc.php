<?php
	...
	//coder71 
	else if(isset($_POST['switchindBtn']))
	{
		$currRep = mysql_real_escape_string($_POST['current_rep']);
		$newRep = mysql_real_escape_string($_POST['new_rep']);
		...
		$i=0;
		foreach($_POST['check'] as $lead=>$on):
			$lead = mysql_real_escape_string($lead);
			$q = "UPDATE leads SET rep_id = $newRep WHERE lead_id = $lead";
			$r = mysql_query($q, $conn);
			$i++;
		endforeach;
		$q1 = "UPDATE invoices SET rep_id = $newRep WHERE rep_id = $currRep";
		$r1 = mysql_query($q1, $conn);
		$msg = "Moved " . $i++ . " leads from $currRepId to $newRepId";
	}
	//coder71
?>
		<div style="float:left;width:25%;">
			<ul>
				...
				<li><a href="#" id="ind_something" class="misc">Move Individual Leads</a></li>
			</ul>
		</div>
					...
					<table cellspacing="5" cellpadding="5">
						...
						<tr>
							<td colspan="2" style="text-align:center;">
								<input type="submit" name="switchBtn" value="Move Leads" />
							</td>
						</tr>
					</table>
				</form>
		...
		<!-- coder71 -->
		<div id="switchInd" style="display:none;">
				<form name="form" method="post" action="">
					<h2 style="text-align:center;">Move Individual Leads</h2>
					<table cellspacing="5" cellpadding="5">
						<tr>
							<td style="text-align:right;">Current Sales Rep:</td>
							<td>
								<select name="current_rep" required>
									<option value=""></option>
									<?php
										$q = "SELECT * FROM reps ORDER BY rep_id ASC";
										$r = mysql_query($q, $conn);
						...
						<tr>
							<td align="top" style="text-align:right;">Client List:</td>
							<td id="showleadshere">Please choose Sales Rep to load Client list</td>
						</tr>
						<tr>
							<td style="text-align:right;">New Sales Rep:</td>
							<td>
						...
						<tr>
							<td colspan="2" style="text-align:center;"><input type="submit" name="switchindBtn" value="Move Individual Leads" /></td>
						</tr>
					</table>
				</form>
			</div>
