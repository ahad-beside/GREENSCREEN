			...
			while($row2 = mysql_fetch_assoc($result)) {
				if($i++ < $count) {
					...
					$q = "SELECT invoices.invoice_id, DATE_FORMAT(invoices.purchase_date, '%c/%d/%y') AS purchase_date, invoices.ship_date, invoices.paid_date, items.quantity, items.price, products.product_code
							...
							" AND WHERE invoices.purchase_date BETWEEN CAST('2016-08-17' AS DATE) AND CAST('2015-01-01' AS DATE) ORDER BY invoices.purchase_date DESC LIMIT 1";
					$r = mysql_query($q, $conn);
					...
					// MOD DATE
					$q = "UPDATE leads SET `last_pulled`='2016-08-26' WHERE lead_id = " . $row2['lead_id'];
					mysql_query($q, $conn);
