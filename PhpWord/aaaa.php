...
<table cellpadding="3" cellspacing="3" style="width: 100%;">
	...
	$sql = "SELECT COUNT(*) AS cn_records FROM leads WHERE `call` != '3' AND (`marks_data` = '0000-00-00' OR `marks_data` <= '$currentDate') AND rep_id = '28'";
	$result_count = mysql_query($sql, $conn);
	$total_records = mysql_fetch_array($result_count);
	$total_records = $total_records['cn_records'];
	$total = ceil($total_records / $records);
	$second_last = $total - 1;

	if(isset($_GET['lead'])) {
        $sql = "SELECT leads.*, reps.name,reps.rep_id AS code FROM leads INNER JOIN reps ON reps.id = leads.rep_id WHERE leads.lead_id = '".intval($_GET['lead'])."' AND leads.call != '3' AND (leads.`marks_data` = '0000-00-00' OR leads.`marks_data` <= '$currentDate') AND leads.rep_id = '28' ORDER BY leads.`rep_id` ASC limit $offset,".$records;
    }else {
        $sql = "SELECT leads.*, reps.name,reps.rep_id AS code FROM leads INNER JOIN reps ON reps.id = leads.rep_id WHERE leads.call != '3' AND (leads.`marks_data` = '0000-00-00' OR leads.`marks_data` <= '$currentDate') AND leads.rep_id = '28' ORDER BY leads.`rep_id` ASC limit $offset,".$records;
...
<tr>	
	...
	<div style="text-align: center;">
		<b style="font-size: 20pt;"><?php echo $business; ?></b></br>
			<center>Lead Owner:&nbsp;&nbsp;<?php echo $rep_name ;?></center>
...
<div style="text-align: center;">
	<form method="GET" action="">
        <div id="div_pagination">
        	...
        	<button type="button" class="house_call" row="<?= $page; ?>" id="<?php echo $lead_id; ?>" name="but_call">Delete</button><?php ?>
 			<button type="button" class="house_sell" row="<?= $page; ?>" id="<?php echo $lead_id; ?>" name="but_sell">Sale/No Sale</button>                
           	</div>
...
<?php 
if($page > 1) {
    echo "<a href='?page=1'><button>First Page</button></a>";
} 
?>
...
<?php 
    if($page < $total) {
        echo "<li><a href='?page=$total'>Last</a></li>";
    } 
?>
</ul>
	       	</div>
